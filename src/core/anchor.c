 ...
 *
 * This file provides the C API defined in include/core/anchor.h.  Every function
 ...
 * Build requirements
 * ------------------
 * 1. Place repy_api.h (from the REPY 2.0.0 release) in include/platform/repy_api.h.
...
#include "platform/recomputils.h" /* also defines NULL and recomp_get_mod_file_path */
#include "core/anchor.h"
#include "player/anchor_player_models.h"
#include "platform/repy_api.h"
                           ...
                           int paused, const char *state_json)
{
    ...
    REPY_FN_SET_S32("paused", paused);
    REPY_FN_SET_STR("state_json", state_json ? state_json : "null");
    REPY_FN_EXEC_CACHE(anchor_impact_update_code,
                      "import anchor_mnsg\n"
                      "result = anchor_mnsg.update_impact(ready, stage, encounter, visit, paused, state_json)\n");
    char *result = REPY_FN_GET_STR("result");
    REPY_FN_CLEANUP;
    return result;
}
...
int anchor_send_impact_hit(int sequence, int amount)
{
    ...
    REPY_FN_SET_S32("sequence", sequence);
    REPY_FN_SET_S32("amount", amount);
    REPY_FN_EXEC_CACHE(anchor_send_impact_hit_code,
                      "import anchor_mnsg\n"
                      "result = anchor_mnsg.send_impact_hit(sequence, amount)\n");
    int result = (int)REPY_FN_GET_BOOL("result");
    REPY_FN_CLEANUP;
    return result;
}
...
{
    REPY_FN_SETUP;
    REPY_FN_EXEC_CACHE(anchor_impact_debug_code,
                      "import anchor_mnsg\n"
                      "result = anchor_mnsg.impact_debug()\n");
    char *result = REPY_FN_GET_STR("result");
    REPY_FN_CLEANUP;
    return result;
}
                                   ...
                                   const char *sample_json)
{
    ...
    REPY_FN_SET_U32("visit", visit);
    REPY_FN_SET_STR("sample_json", sample_json ? sample_json : "null");
    REPY_FN_EXEC_CACHE(anchor_impact_sounds_update_code,
                      "import anchor_mnsg\n"
                      "result = anchor_mnsg.update_impact_sounds(ready, stage, encounter, visit, sample_json)\n");
    char *result = REPY_FN_GET_STR("result");
    REPY_FN_CLEANUP;
    return result;
}
...
char *anchor_update_world_quest(const char *sources, const char *status)
{
    REPY_FN_SETUP;
    REPY_FN_SET_STR("sources", sources ? sources : "{}");
    REPY_FN_SET_STR("status", status ? status : "{}");
    REPY_FN_EXEC_CACHE(anchor_world_quest_update_code,
        "import anchor_mnsg\n"
        "result = anchor_mnsg.update_world_quest(sources, status)\n");
    char *result = REPY_FN_GET_STR("result");
    REPY_FN_CLEANUP;
    return result;
}
