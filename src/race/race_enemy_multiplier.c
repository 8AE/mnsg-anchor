...
/* The race multiplier passes a stack copy of the placed source to the native
 * constructor. That copy is absent from the guarded world roster, so retain
 * its explicit placement and duplicate ordinal until the actor pool reuses
 * the slot. The generation byte rejects stale entries after such reuse. */
typedef struct {
    void *actor;
    unsigned short room, parent, ordinal;
    unsigned char generation;
} RaceBoulderDuplicate;
static RaceBoulderDuplicate s_boulder_duplicates[128];
static unsigned int s_boulder_duplicate_next;

void anchor_race_boulder_room_reset(void)
{
    unsigned int i;
    for (i = 0; i < 128; ++i) s_boulder_duplicates[i].actor = 0;
    s_boulder_duplicate_next = 0;
}

void anchor_race_boulder_forget(void *actor)
{
    unsigned int i;
    for (i = 0; i < 128; ++i)
        if (s_boulder_duplicates[i].actor == actor)
            s_boulder_duplicates[i].actor = 0;
}

int anchor_race_boulder_duplicate(void *actor, unsigned int *parent,
                                  unsigned int *ordinal)
{
    unsigned int i;
    if (!actor) return 0;
    for (i = 0; i < 128; ++i) {
        const RaceBoulderDuplicate *d = &s_boulder_duplicates[i];
        if (d->actor == actor && d->room == D_800C7AB2 &&
            d->generation == *((unsigned char *)actor + 0x74)) {
            if (parent) *parent = d->parent;
            if (ordinal) *ordinal = d->ordinal;
            return 1;
        }
    }
    return 0;
...
void anchor_race_enemy_multiplier_spawn_hook(int actor, RaceEnemyActorInstance *instance)
{
    ...
    short z_offset;
    float scale;
    RaceBoulderDuplicate *registered;
    ...
    if (!is_enemy_actor_id(actor_id))
        return;
    if (actor_id == 0x3DA)
        anchor_world_source_index(instance, &boulder_parent);
    ...
    for (duplicate_index = 1; duplicate_index < multiplier; duplicate_index++)
    {
        ...
        if (actor_id == 0x3DA && boulder_parent) {
            RaceBoulderDuplicate *d =
                &s_boulder_duplicates[s_boulder_duplicate_next++ % 128];
            d->actor = (void *)(unsigned long)(unsigned int)duplicate;
            d->room = D_800C7AB2;
            d->parent = (unsigned short)boulder_parent;
            d->ordinal = (unsigned short)duplicate_index;
            d->generation = *((unsigned char *)d->actor + 0x74);
            registered = d;
        }
        ...
        if (registered)
            registered->generation = *((unsigned char *)registered->actor + 0x74);
    }
}
