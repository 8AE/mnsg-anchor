#include "bosses/impact/anchor_impact_codec.h"
...
static int snapshot(JsonCursor *c, AnchorImpactNativeSnapshot *out)
{
    char key[8];
    ...
    for (;;)
    {
        ...
        {
            bit = 1;
            if (!words(c, out->root, ANCHOR_IMPACT_ROOT_WORDS))
                return 0;
        }
        else if (same(key, "k"))
        {
            bit = 2;
            if (!number(c, &out->encounter))
                return 0;
        }
        else if (same(key, "s"))
        {
            bit = 4;
            if (!number(c, &out->stage))
                return 0;
        }
        else
            return 0;
...
static int hits(JsonCursor *c, AnchorImpactStatus *out)
{
    int i, j;
    ...
    if (c->p < c->end && *c->p == ']')
        return token(c, ']');
    for (i = 0; i < ANCHOR_IMPACT_HIT_BATCH; ++i)
    {
...
int anchor_impact_status_decode(const char *json, AnchorImpactStatus *out)
{
    ...
    if (!json || !out)
        return 0;
    while (length < ANCHOR_IMPACT_STATUS_JSON_SIZE && json[length])
        ++length;
    if (length == ANCHOR_IMPACT_STATUS_JSON_SIZE)
        return 0;
    c.p = json;
    c.end = json + length;
    out->has_state = 0;
                               ...
                               char *out, unsigned int capacity)
{
    ...
    literal(&w, "],\"k\":");
    integer(&w, state->encounter);
    literal(&w, ",\"s\":");
    integer(&w, state->stage);
    literal(&w, "}");
    *w.p = 0;
    if (!w.valid)
        out[0] = 0;
