...
static int projectiles(JsonCursor *c, AnchorDharumanyoNativeSnapshot *out)
{
    unsigned int i, j;
...
static int snapshot(JsonCursor *c, AnchorDharumanyoNativeSnapshot *out)
{
    char key[8];
    ...
    for (;;)
    {
        ...
        {
            bit = 1;
            if (!words(c, out->root, ANCHOR_DHARUMANYO_ROOT_WORDS))
                return 0;
        }
        else if (same(key, "c"))
        {
            bit = 2;
            if (!words(c, out->carrier, ANCHOR_DHARUMANYO_CARRIER_WORDS))
                return 0;
        }
        else if (same(key, "s"))
        {
...
static int hits(JsonCursor *c, AnchorDharumanyoStatus *out)
{
    int i, j;
    ...
    if (c->p < c->end && *c->p == ']')
        return token(c, ']');
    for (i = 0; i < ANCHOR_DHARUMANYO_HIT_BATCH; ++i)
    {
                                     ...
                                     AnchorDharumanyoStatus *out)
{
    ...
    if (!json || !out)
        return 0;
    while (length < ANCHOR_DHARUMANYO_STATUS_JSON_SIZE && json[length])
        ++length;
    if (length == ANCHOR_DHARUMANYO_STATUS_JSON_SIZE)
        return 0;
    c.p = json;
    c.end = json + length;
    out->has_state = 0;
                                    ...
                                    char *out, unsigned int capacity)
{
    ...
    if (!out || !capacity)
        return 0;
    out[0] = 0;
    if (!state || state->projectile_count > ANCHOR_DHARUMANYO_MAX_PROJECTILES)
        return 0;
    w.p = out;
