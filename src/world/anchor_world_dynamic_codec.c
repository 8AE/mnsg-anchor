...
int anchor_world_dynamic_row_valid(const int *r) {
  ...
  for (i = 0; i < WORLD_DYNAMIC_WORDS; ++i) {
    ...
    if ((r[WD_KIND]==WD_SLICER || r[WD_KIND]==WD_RANDOM || r[WD_KIND]==WD_BOMB || r[WD_KIND]==WD_WAVE || r[WD_KIND]==WD_FRAGILE || r[WD_KIND]==WD_BOULDER || r[WD_KIND]==WD_FISH) &&
        (i==WS_INSTANCE || i==WS_RECEIPT)) { min=0;max=2147483647; }
    if ((r[WD_CID]==WF_LOOT_ORIGIN ||
         (!r[WD_CID] && (r[WD_BASE_Y]==4 || r[WD_BASE_Y]==5))) &&
        r[WD_PARENT]>=1 && r[WD_PARENT]<=256 &&
        r[WD_ORDINAL]==r[WD_BASE_Y]-4 && r[WD_KIND]>=WD_COIN &&
        r[WD_KIND]<=WD_FOOD && i==WD_BASE_Y) { min=4;max=5; }
  ...
  if (r[WD_KIND]==WD_FRAGILE)
    return r[WD_SERIAL]>0 && anchor_world_fragile_valid(r);
  if (r[WD_KIND]==WD_BOULDER)
    return r[WD_SERIAL]>0 && anchor_world_boulder_valid(r);
  if (r[WD_KIND]==WD_FISH)
    return r[WD_SERIAL]>0 && anchor_world_fish_valid(r);
  if (r[WD_CID]==WF_LOOT_ORIGIN &&
      (r[WD_KIND]<WD_COIN || r[WD_KIND]>WD_FOOD ||
       r[WD_PARENT]<1 || r[WD_PARENT]>256 ||
       (r[WD_BASE_Y]!=4 && r[WD_BASE_Y]!=5) ||
       r[WD_ORDINAL]!=r[WD_BASE_Y]-4)) return 0;
