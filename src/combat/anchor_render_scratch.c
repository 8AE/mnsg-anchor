...
void anchor_render_scratch_begin_object(void *pointer)
{
    ...
    if (!anchor_player_models_is_remote_object(object) ||
        ...
        return;
    /* Every remote draw requires a budgeted scratch redirect. Keep the object
     * hidden for this native draw if the redirect cannot be prepared, then
     * restore its prior visibility in the matching end hook. */
    s_object = object;
