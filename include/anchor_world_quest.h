 ...
 * (family, role, parent roster slot, activation ordinal, native generation).
...
#define WORLD_QUEST_JSON 65536

/* Row ABI. Increment on any field, range or role change. */
#define WORLD_QUEST_ABI 1

/* Field order of one record. 40..61 are reserved and must stay zero so a
 * future field can be added without moving INSTANCE/RECEIPT. */
enum {
  ...
  WQ_GEN,         /* native task generation, actor+0x74, 0..255 */
  ...
  WQ_SLOT,        /* native static slot, 0..2 */
  ...
  WQ_COLOUR,      /* raw object+0x30 colour/primitive word */
  ...
  WQ_MESH_READY,  /* 1 when the mesh private block exists and is neutralised */
  WQ_TEXTURE,     /* raw object+0x7C local render/texture flag byte */
  WQ_FREE0 = 40,
  WQ_INSTANCE = 62, /* per-record instance id assigned by the transport */
  WQ_RECEIPT = 63   /* acknowledgement word; never captured locally */
};
...
enum {
  ...
  WQ_ROLE_GMC74_ROOT = 11,     /* File_74 placed root 0x35C */
  WQ_ROLE_GMC74_CHILD = 12,    /* one of the 23 staged children */
  WQ_ROLE_GMC75_ROOT = 13,     /* File_75 placed root 0x35D */
  WQ_ROLE_GMC75_CHILD = 14     /* File_75 single staged child */
};
...
/* File_74 spawns 23 staged children after its first child (slot +0x48). */
#define WORLD_QUEST_GMC74_CHILDREN 23
